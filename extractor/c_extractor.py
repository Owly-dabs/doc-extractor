...
class CDocstringExtractor(DocstringExtractor):
    ...
    def extract_docstrings(self, code: str) -> List[Dict]:
        ...
        def get_node_name(node):
            """Recursively find the most likely symbol name."""
            if node.type in ["identifier", "field_identifier"]:
                return get_node_text(node)

            # Special case: typedef struct config_s { ... } config;
            if node.type == "type_definition":
                # Look for struct_specifier and return its identifier
            ...
            # Prioritize function_declarator if present
            for child in node.children:
